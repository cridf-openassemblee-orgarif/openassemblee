...
angular
    ...
    .controller(
        "SeanceDetailController",
        function ($scope, $rootScope, $stateParams, entity, Seance, $http) {
            $scope.dto = entity;
            ...
            var initGroupePolitiques = function () {
                ...
                if ($scope.dto.pouvoirs) {
                    $scope.dto.pouvoirs.map(function (p) {
                        if (
                            p.eluCedeur.groupePolitique != null &&
                            groupesPolitiques[p.eluCedeur.groupePolitique.id] ==
                                null
                        ) {
                            groupesPolitiques[p.eluCedeur.groupePolitique.id] =
                                p.eluCedeur.groupePolitique;
                        }
                        if (
                            p.eluBeneficiaire.groupePolitique != null &&
                            groupesPolitiques[
                                p.eluBeneficiaire.groupePolitique.id
                            ] == null
                        ) {
                            groupesPolitiques[
                                p.eluBeneficiaire.groupePolitique.id
                            ] = p.eluBeneficiaire.groupePolitique;
                        }
                    });
                }
            ...
            var initSignaturesArray = function () {
                if (entity.seance) {
                    ...
                    $scope.signaturesOptions.signaturesNumber =
                        entity.seance.nombreSignatures;
                }
            };
            ...
            $scope.updateSignaturesCount = function () {
                if (entity.seance) {
                    ...
                    var awaited =
                        entity.seance.presenceElus.length *
                        entity.seance.nombreSignatures;
                    var stats = {};
                    ...
                    entity.groupePolitiques.forEach(function (gp) {
                        expectedSignatures[gp.nomCourt] =
                            entity.seance.presenceElus.filter(function (pe) {
                                return pe.elu.groupePolitique === gp.nomCourt;
                            }).length;
                    });
                    expectedSignatures[SANS_GROUPE] =
                        entity.seance.presenceElus.filter(function (pe) {
                    ...
                    for (var i = 1; i <= entity.seance.nombreSignatures; i++) {
                        stats[i]["missing"] =
                            ...
                            (stats[i]["PRESENT"] +
                                ...
                                stats[i]["EXCUSE"]);
                        entity.groupePolitiques.forEach(function (gp) {
                            stats[gp.nomCourt][i]["missing"] =
                                expectedSignatures[gp.nomCourt] -
                                (stats[gp.nomCourt][i]["PRESENT"] +
                                    stats[gp.nomCourt][i]["ABSENT"] +
                                    stats[gp.nomCourt][i]["EXCUSE"]);
                        });
                        stats[SANS_GROUPE][i]["missing"] =
                            expectedSignatures[SANS_GROUPE] -
                            (stats[SANS_GROUPE][i]["PRESENT"] +
                                stats[SANS_GROUPE][i]["ABSENT"] +
                                stats[SANS_GROUPE][i]["EXCUSE"]);
                    }
