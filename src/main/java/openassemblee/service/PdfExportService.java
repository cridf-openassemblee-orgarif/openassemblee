...
public class PdfExportService {
    ...
    public byte[] exportCommissionsThematiques(List<CommissionThematique> commissionThematiques,
                                               Map<Long, List<EluEnFonctionDTO>> appartenancesByCt,
                                               Map<Long, List<EluEnFonctionDTO>> fonctionsByCt) throws DocumentException {
        Document document = new Document();
        ByteArrayOutputStream output = new ByteArrayOutputStream();
        PdfWriter.getInstance(document, output);
        ...
        commissionThematiques.forEach(ct -> {
            try {
                document.add(subTitle1("Commission thématique \"" + ct.getNom() + "\""));
    ...
    public byte[] exportCommissionsThematique(CommissionThematique commissionThematique,
                                              List<EluEnFonctionDTO> appartenances,
                                              List<EluEnFonctionDTO> fonctions) throws DocumentException {
        Document document = new Document();
        ByteArrayOutputStream output = new ByteArrayOutputStream();
        PdfWriter.getInstance(document, output);
...
//        addHeader(table, "Statut");
    ...
    private void addElu(Document document, EluListDTO dto) throws DocumentException {
        Elu elu = dto.getElu();
        Paragraph title = bigP(elu.getCiviliteLabel() + " " + elu.getPrenom() + " " + elu.getNom());
        title.setSpacingBefore(10f);
        document.add(title);
        document.add(p("Date de naissance : " + (elu.getDateNaissance() != null ? dateFormatter.format(elu.getDateNaissance()) : "Inconnue")));
...
//        document.add(p("Groupe politique : " + elu.getAppartenancesGroupePolitique().get(0).getGroupePolitique().getNom()));
        ...
        if (elus != null) {
            elus.stream()
                .forEach(e -> {
                    try {
                        Elu elu = e.getElu();
                        String fonctionLabel = e.getFonctionLabel() != null ? e.getFonctionLabel() + " : " : "";
                        Paragraph title = bigP(fonctionLabel + elu.getCiviliteLabel() + " " + elu.getPrenom() + " " + elu.getNom());
                        title.setSpacingBefore(10f);
                        document.add(title);
                    } catch (DocumentException ex) {
                        throw new RuntimeException(ex);
    ...
    private void addCpElu(Document document, EluEnFonctionDTO dto) throws DocumentException {
        Elu elu = dto.getElu();
        Paragraph title = bigP(elu.getCiviliteLabel() + " " + elu.getPrenom() + " " + elu.getNom());
        title.setSpacingBefore(10f);
        document.add(title);
...
//        chunk.setFont(font);
    ...
    private PdfPTable adressePostales(List<AdressePostale> aps) throws DocumentException {
    ...
    private PdfPTable mails(List<AdresseMail> mails) throws DocumentException {
        ...
        mails
            ...
            .forEach(m -> {
                table.addCell(m.getNatureProPerso() != null ? m.getNatureProPerso().name() : "");
                table.addCell(m.getNiveauConfidentialite() != null ? m.getNiveauConfidentialite().name() : "");
    ...
    private PdfPTable numeros(List<NumeroTelephone> numeros) throws DocumentException {
        PdfPTable table = new PdfPTable(3);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{1, 1, 1});
        ...
        numeros
            ...
            .forEach(n -> {
                table.addCell(n.getNatureProPerso() != null ? n.getNatureProPerso().name() : "");
                table.addCell(n.getNiveauConfidentialite() != null ? n.getNiveauConfidentialite().name() : "");
