...
public class HemicycleConfigurationRendererService {
    ...
    public HemicycleConfigurationRendu hemicycle(HemicycleConfigurationDefinition hd) {
        List<ChairRendu> chairs = hd.rows.stream()
        ...
        List<ChairRendu> numberedChairs = IntStream.range(0, chairs.size())
            .mapToObj(i -> {
        ...
        return new HemicycleConfigurationRendu(numberedChairs,
            minX - hd.margin, minY - hd.margin,
            (maxX - minX + 2 * hd.margin), (maxY - minY + 2 * hd.margin), numbers.get(0),
            numbers.get(numbers.size() - 1));
    }

    private List<ChairRendu> calculateChairs(List<RowDefinition> rows, HemicycleConfigurationDefinition hd) {
        if (rows.size() != 3 ||
    ...
    private ChairRendu calcChair(double lineBaseX, double lineBaseY, int chairPosition, int offset, double largeur,
        ...
        return new ChairRendu(chairNumber,
            baseX,
            baseY,
            baseX2,
            baseY,
    ...
    private ChairRendu translateCenter(ChairRendu d, HemicycleConfigurationDefinition hd) {
        double x = hd.baseX;
        double y = hd.baseY;
        return new ChairRendu(d.number,
            x + d.baseX1,
            y - d.baseY1,
            x + d.baseX2,
            y - d.baseY2,
    ...
    private List<ChairRendu> getFrontChairs(HemicycleConfigurationDefinition hd, int baseY) {
