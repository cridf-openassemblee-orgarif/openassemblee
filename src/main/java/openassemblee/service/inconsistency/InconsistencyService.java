...
//        List<Pouvoir> missingBeneficiaire =
        inconsistencies.add(new InconsistenciesDTO(InconsistenciesDTO.InconsistencyCategory.POUVOIR_BENEFICIAIRE_MANQUANT,
            pouvoirs
                .stream()
                .filter(p -> p.getEluBeneficiaire() == null)
                .map(this::pouvoirInconsistency)
                .collect(Collectors.toList())));
        inconsistencies.add(new InconsistenciesDTO(InconsistenciesDTO.InconsistencyCategory.POUVOIR_CEDEUR_MANQUANT,
            pouvoirs
                .stream()
    ...
    private InconsistencyDTO pouvoirInconsistency(Pouvoir p) {
        return new InconsistencyDTO(p.getClass(), p.getId(), Seance.class, p.getSeance().getId());
    }
}
