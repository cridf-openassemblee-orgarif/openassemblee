...
public class SearchService {
    ...
    public void resetIndex() {
        ...
        resetRepository(
            groupePolitiqueRepository,
            groupePolitiqueSearchRepository
        );
        resetRepository(
            commissionThematiqueRepository,
            commissionThematiqueSearchRepository
        );
        resetRepository(
            fonctionExecutiveRepository,
            fonctionExecutiveSearchRepository
        );
    }

    private <T> void resetRepository(
    ...
    public List<SearchResultDTO> search(String searchToken) {
        ...
        results.addAll(
            StreamSupport
                ...
                .filter(e ->
                    isCurrentMandat(
                        eluRepository.getOne(e.getId()).getMandats(),
                        mandature
                    )
                )
                .map(e ->
        ...
        if (results.size() < 20) {
            results.addAll(
                StreamSupport
                    ...
                    .filter(gp ->
                        gp.getMandature().getId().equals(mandature.getId())
                    )
                    .map(e ->
                        new SearchResultDTO(
                            GROUPE_POLITIQUE,
            ...
            if (results.size() < 20) {
                results.addAll(
                    StreamSupport
                        ...
                        .filter(ct ->
                            ct.getMandature().getId().equals(mandature.getId())
                        )
                        .map(e ->
                            new SearchResultDTO(
                ...
                if (results.size() < 20) {
                    results.addAll(
                        StreamSupport
                            ...
                            .filter(e ->
                                isCurrentMandat(e.getMandats(), mandature)
                            )
                            .map(e ->
                                new SearchResultDTO(
                                    ELU,
