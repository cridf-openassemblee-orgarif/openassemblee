...
public class EluService {
                ...
                for (AppartenanceCommissionPermanente a : e.getAppartenancesCommissionPermanente()) {
                    if (
                        ...
                        a
                            ...
                            .getId()
                            .equals(
                                sessionMandatureService.getMandature().getId()
                            )
                    ) {
                        return true;
                    }
    ...
    public List<EluListDTO> getAll(
        ...
        return elus
            ...
            .filter(e ->
                !removeDemissionaires ||
                isCurrentMandat(e.getElu().getMandats(), currentMandature)
            )
    ...
    @Transactional(readOnly = true)
    public EluListDTO getEluListDTO(
        Long id,
        Boolean loadAdresses,
        Boolean filterAdresses
    ) {
        return eluToEluListDTO(
            eluRepository.findOne(id),
            loadAdresses,
            filterAdresses
        );
    }

    public EluListDTO eluToEluListDTO(
        Elu elu,
        Boolean loadAdresses,
        Boolean filterAdresses
    ) {
        Optional<GroupePolitique> groupePolitique = elu
            ...
            .filter(a ->
                a
                    ...
                    .getMandature()
                    .getId()
                    .equals(sessionMandatureService.getMandature().getId())
            )
            .map(AppartenanceGroupePolitique::getGroupePolitique)
            .filter(Objects::nonNull)
            .findFirst();
        String fonctionExec = elu
            ...
            .findFirst()
            .map(f ->
                FonctionCommissionPermanente.getShortFonction(f.getFonction())
            )
            .orElse(null);
        String fonctionCP = elu
            ...
            .orElse(null);
        String shortFonction = fonctionExec != null
            ? fonctionExec
            : fonctionCP != null ? fonctionCP : null;
        Mandat currentMandat = getOnlyCurrentMandat(
            elu.getMandats(),
            sessionMandatureService.getMandature()
        );
        Boolean actifInCurrentMandat =
            currentMandat != null && currentMandat.getDateDemission() == null;
        if (groupePolitique.isPresent()) {
            return new EluListDTO(
                elu,
                groupePolitique.get(),
                shortFonction,
                actifInCurrentMandat,
                loadAdresses,
                filterAdresses
            );
        } else {
    ...
    public EluDTO get(Long id, Boolean filterAdresses) {
        ...
        List<GroupePolitique> groupesPolitiques = elu
            .getAppartenancesGroupePolitique()
            .stream()
            .map(i -> i.getGroupePolitique())
            .collect(Collectors.toList());
        groupesPolitiques.addAll(
            elu
        ...
        Map<Long, GroupePolitique> groupesPolitiquesMap = groupesPolitiques
            .stream()
            .distinct()
            .collect(
                Collectors.toMap(GroupePolitique::getId, Function.identity())
            );
        List<CommissionThematique> commissionThematiques = elu
            .getAppartenancesCommissionsThematiques()
            .stream()
            .map(i -> i.getCommissionThematique())
            .collect(Collectors.toList());
        commissionThematiques.addAll(
            elu
        ...
        Map<Long, CommissionThematique> commissionThematiquesMap =
            commissionThematiques
                .stream()
                .distinct()
                .collect(
                    Collectors.toMap(
                        CommissionThematique::getId,
                        Function.identity()
                    )
                );
        return new EluDTO(
            elu,
            groupesPolitiquesMap,
            commissionThematiquesMap,
            true,
            filterAdresses
        );
    }
    ...
    void filterEluMandat(Elu elu, Mandature mandature) {
        ...
        elu.setAppartenancesCommissionPermanente(
            elu
                .getAppartenancesCommissionPermanente()
                .stream()
                .filter(a -> a.getMandature().getId().equals(mandatureId))
                .collect(Collectors.toList())
        );
        elu.setAppartenancesCommissionsThematiques(
            elu
                .getAppartenancesCommissionsThematiques()
                .stream()
        ...
        elu.setAppartenancesGroupePolitique(
            elu
                .getAppartenancesGroupePolitique()
                .stream()
                .filter(a -> a.getGroupePolitique() != null)
        ...
        elu.setFonctionsCommissionPermanente(
            elu
                .getFonctionsCommissionPermanente()
                .stream()
                .filter(f -> f.getMandature().getId().equals(mandatureId))
                .collect(Collectors.toList())
        );
        elu.setFonctionsExecutives(
            elu
                .getFonctionsExecutives()
                .stream()
                .filter(f -> f.getMandature().getId().equals(mandatureId))
        ...
        elu.setFonctionsGroupePolitique(
            elu
                .getFonctionsGroupePolitique()
                .stream()
        ...
        elu.setFonctionsCommissionsThematiques(
            elu
                .getFonctionsCommissionsThematiques()
                .stream()
    ...
    @Transactional
    public void saveIdentiteInternet(
        Long id,
        IdentiteInternet identiteInternet
    ) {
    ...
    public List<String> xlsEluLine(
        Elu e,
        GroupePolitique gp,
        Boolean demissionaire
    ) {
        String civilite = e.getCiviliteLabel();
        String groupePolitique = gp != null
            ? gp.getNom()
        ...
        String emails = e
            ...
            .map(ap -> ap.getMail())
            .reduce("", (a1, a2) -> a1 + (a1.equals("") ? "" : ", ") + a2);
        String numerosPublics = e
            .getNumerosTelephones()
            .stream()
            .filter(n ->
                n.getNiveauConfidentialite() == NiveauConfidentialite.PUBLIABLE
            )
            .map(n ->
                n.getNumero() +
                (
        ...
        String numerosInternesOuConfidentiels = e
            .getNumerosTelephones()
            .stream()
            .filter(n ->
                n.getNiveauConfidentialite() == NiveauConfidentialite.INTERNE ||
        ...
        if (!demissionaire) {
            return Arrays.asList(
                ...
                emails,
                numerosPublics,
                numerosInternesOuConfidentiels
            );
        } else {
            String dateDemission = mandat != null &&
                mandat.getDateDemission() != null
                ? mandat
                    .getDateDemission()
                    .format(DateTimeFormatter.ISO_LOCAL_DATE)
                : "";
            String motifDemission = mandat != null
                ? mandat.getMotifDemission()
                : "";
            return Arrays.asList(
                civilite,
                e.getPrenom(),
    ...
    @Transactional(readOnly = true)
    public static boolean isCurrentMandat(
        Set<Mandat> mandats,
    ...
    @Transactional(readOnly = true)
    private static List<Mandat> getCurrentMandats(
        Set<Mandat> mandats,
        Mandature currentMandature
    ) {
        ...
        return mandats
            .stream()
            .filter(m ->
                m.getMandature().getId().equals(currentMandature.getId())
            )
            .filter(m ->
                Strings.isNullOrEmpty(m.getMotifDemission()) &&
                m.getDateDemission() == null
            )
            .collect(Collectors.toList());
    }
    ...
    @Transactional(readOnly = true)
    public static Mandat getOnlyCurrentMandat(
        Set<Mandat> mandats,
        Mandature currentMandature
    ) {
