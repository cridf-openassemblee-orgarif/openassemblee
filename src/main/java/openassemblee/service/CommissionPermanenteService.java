...
public class CommissionPermanenteService {
    ...
    public ExecutifDTO getExecutif(Boolean removeDemissionaires) {
        ...
        List<FonctionExecutive> fe = fonctionExecutiveRepository
        ...
        Set<Elu> elusFeIds = fe
            .stream()
            .map(FonctionExecutive::getElu)
            .collect(Collectors.toSet());
        List<FonctionCommissionPermanente> fcp =
            fonctionCommissionPermanenteRepository
                ...
                .filter(f -> !removeDemissionaires || f.getDateFin() == null)
                .sorted(
                    EluNomComparator.comparing(
                        FonctionCommissionPermanente::getElu
                    )
                )
                .collect(Collectors.toList());
        ...
        Map<Long, Elu> elus = eluRepository
            .findAll(elusIds)
            .stream()
            .collect(Collectors.toMap(Elu::getId, Function.identity()));
        return new ExecutifDTO(fcp, fe, elus);
    }

    @Transactional(readOnly = true)
    public CommissionPermanenteDTO getCommissionPermanente() {
        List<AppartenanceCommissionPermanente> acp =
            appartenanceCommissionPermanenteRepository
                ...
                .sorted(
                    EluNomComparator.comparing(
                        AppartenanceCommissionPermanente::getElu
                    )
                )
                .collect(Collectors.toList());
        Set<Long> elusIds = new HashSet<>();
    ...
    @Transactional(readOnly = true)
    public List<EluEnFonctionDTO> getFonctionExecutivesCommissionPermanenteDtos(
        Boolean filterAdresses
    ) {
        return fonctionExecutiveRepository
            .findByMandature(sessionMandatureService.getMandature())
            .stream()
            .filter(CommissionPermanenteService::isFonctionExecutiveCourante)
            .sorted(this::sortFonctionExecutives)
            .map(f -> {
                EluListDTO eluDTO = eluService.eluToEluListDTO(
                    f.getElu(),
                    true,
                    filterAdresses
                );
    ...
    @Transactional(readOnly = true)
    public List<EluEnFonctionDTO> getAppartenancesCommissionPermanenteDtos(
        Boolean filterAdresses,
        Boolean removeDemissionaires
    ) {
        return appartenanceCommissionPermanenteRepository
            .findByMandature(sessionMandatureService.getMandature())
            .stream()
            .filter(this::isAppartenanceCourante)
    ...
    private List<List<String>> getFonctionExecutivesLines(
        List<FonctionExecutive> fes
    ) {
        ...
        for (FonctionExecutive fe : fes) {
            ...
            String dateDebut = fe.getDateDebut() != null
                ? fe.getDateDebut().format(DateTimeFormatter.ISO_LOCAL_DATE)
    ...
    private List<List<String>> getAppartenancesLines(
        ...
        for (AppartenanceCommissionPermanente acp : acps) {
            ...
            String dateDebut = acp.getDateDebut() != null
                ? acp.getDateDebut().format(DateTimeFormatter.ISO_LOCAL_DATE)
                : "Date de début inconnue";
            String dateFin = acp.getDateFin() != null
                ? acp.getDateFin().format(DateTimeFormatter.ISO_LOCAL_DATE)
                : null;
            line.add(dateDebut);
            line.add(dateFin);
    ...
    private List<List<String>> getFonctionsLines(
        List<FonctionCommissionPermanente> fcps
    ) {
        List<List<String>> lines = new ArrayList<>();
        lines.add(
            Arrays.asList(
        ...
        for (FonctionCommissionPermanente fcp : fcps) {
            ...
            String dateDebut = fcp.getDateDebut() != null
                ? fcp.getDateDebut().format(DateTimeFormatter.ISO_LOCAL_DATE)
                : "Date de début inconnue";
    ...
    private List<String> getEluLine(
        Elu elu,
        ...
        String groupePolitique = dto.getGroupePolitique() != null
            ? dto.getGroupePolitique().getNom()
            : "Aucun groupe politique";
        String dateNaissance = elu.getDateNaissance() != null
            ? elu.getDateNaissance().format(DateTimeFormatter.ISO_LOCAL_DATE)
    ...
    public ExcelExportService.Entry[] getExecutifExportEntries() {
        ...
        ExcelExportService.Entry[] entries = new ExcelExportService.Entry[] {
            new ExcelExportService.Entry(
                "Exécutif",
                getFonctionExecutivesLines(dto.getFonctionsExecutives())
            ),
            new ExcelExportService.Entry(
                "Fonctions",
                getFonctionsLines(dto.getFonctions())
            ),
        };
        return entries;
    }
    ...
    public ExcelExportService.Entry[] getCommissionPermanenteExportEntries() {
        ...
        List<AppartenanceCommissionPermanente> acps = elus
            .stream()
    ...
    public List<List<String>> getFonctionsEntry(List<EluListDTO> dtos) {
        List<FonctionCommissionPermanente> fcps = dtos
            .stream()
            .map(EluListDTO::getElu)
            .flatMap(e -> e.getFonctionsCommissionPermanente().stream())
            .collect(Collectors.toList());
        return getFonctionsLines(fcps);
    }

    public int sortFonctionExecutives(
        FonctionExecutive fe1,
        FonctionExecutive fe2
    ) {
    ...
    public int fonctionScore(FonctionExecutive fe) {
        if (fe.getFonction().startsWith("Président")) {
            ...
            Scanner scanner = new Scanner(fe.getFonction())
                .useDelimiter("[^0-9]+");
