...
public class HemicycleConfigurationResource {
    ...
    private final Logger log = LoggerFactory.getLogger(
        HemicycleConfigurationResource.class
    );
     ...
     */
    @RequestMapping(
        value = "/hemicycleConfigurations",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public ResponseEntity<HemicycleConfiguration> createHemicycleConfiguration(
        @Valid @RequestBody HemicycleConfiguration hemicycleConfiguration
    ) throws URISyntaxException {
        log.debug(
            "REST request to save HemicycleConfiguration : {}",
        ...
        if (hemicycleConfiguration.getId() != null) {
            return ResponseEntity
                ...
                .header(
                    "Failure",
                    "A new hemicycleConfiguration cannot already have an ID"
                )
                .body(null);
        }
        HemicycleConfiguration result = hemicycleConfigurationRepository.save(
            hemicycleConfiguration
        );
        hemicycleConfigurationSearchRepository.save(result);
        return ResponseEntity
            .created(new URI("/api/hemicycleConfigurations/" + result.getId()))
            .headers(
    ...
    @Timed
    public ResponseEntity<HemicycleConfiguration> updateHemicycleConfiguration(
        @Valid @RequestBody HemicycleConfiguration hemicycleConfiguration
    ) throws URISyntaxException {
        log.debug(
            "REST request to update HemicycleConfiguration : {}",
            hemicycleConfiguration
        );
        if (hemicycleConfiguration.getId() == null) {
        ...
        return ResponseEntity
            ...
            .headers(
                HeaderUtil.createEntityUpdateAlert(
                    "hemicycleConfiguration",
                    hemicycleConfiguration.getId().toString()
                )
            )
            .body(result);
    }
     ...
     */
    @RequestMapping(
        value = "/hemicycleConfigurations/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public ResponseEntity<HemicycleConfiguration> getHemicycleConfiguration(
        @PathVariable Long id
    ) {
        log.debug("REST request to get HemicycleConfiguration : {}", id);
        return Optional
            .ofNullable(hemicycleConfigurationRepository.findOne(id))
            .map(hemicycleConfiguration ->
                new ResponseEntity<>(hemicycleConfiguration, HttpStatus.OK)
            )
    ...
    @Timed
    public ResponseEntity<Void> deleteHemicycleConfiguration(
        @PathVariable Long id
    ) {
     ...
     */
    @RequestMapping(
        value = "/_search/hemicycleConfigurations/{query}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public List<HemicycleConfiguration> searchHemicycleConfigurations(
        @PathVariable String query
    ) {
        return StreamSupport
            .stream(
                hemicycleConfigurationSearchRepository
