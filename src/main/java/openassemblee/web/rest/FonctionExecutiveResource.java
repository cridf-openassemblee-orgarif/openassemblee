...
public class FonctionExecutiveResource {
    ...
    private final Logger log = LoggerFactory.getLogger(
        FonctionExecutiveResource.class
    );
     ...
     */
    @RequestMapping(
        value = "/fonctionExecutives",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public ResponseEntity<FonctionExecutive> createFonctionExecutive(
        @RequestBody FonctionExecutive fonctionExecutive
    ) throws URISyntaxException {
        log.debug(
            "REST request to save FonctionExecutive : {}",
            fonctionExecutive
        );
        if (fonctionExecutive.getId() != null) {
            return ResponseEntity
                ...
                .header(
                    "Failure",
                    "A new fonctionExecutive cannot already have an ID"
                )
                .body(null);
        }
        FonctionExecutive result = fonctionExecutiveRepository.save(
            fonctionExecutive
        );
        fonctionExecutiveSearchRepository.save(result);
        auditTrailService.logCreation(result, result.getId());
        return ResponseEntity
            .created(new URI("/api/fonctionExecutives/" + result.getId()))
            .headers(
    ...
    @Timed
    public ResponseEntity<FonctionExecutive> updateFonctionExecutive(
        @RequestBody FonctionExecutive fonctionExecutive
    ) throws URISyntaxException {
        log.debug(
            "REST request to update FonctionExecutive : {}",
            fonctionExecutive
        );
        if (fonctionExecutive.getId() == null) {
        ...
        return ResponseEntity
            ...
            .headers(
                HeaderUtil.createEntityUpdateAlert(
                    "fonctionExecutive",
                    fonctionExecutive.getId().toString()
                )
            )
            .body(result);
    }
    ...
    public List<FonctionExecutive> getAllFonctionExecutives() {
        ...
        return fonctionExecutiveRepository.findByMandature(
     ...
     */
    @RequestMapping(
        value = "/fonctionExecutives/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public ResponseEntity<FonctionExecutive> getFonctionExecutive(
     ...
     */
    @RequestMapping(
        value = "/_search/fonctionExecutives/{query}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public List<FonctionExecutive> searchFonctionExecutives(
        @PathVariable String query
    ) {
        return Lists.newArrayList(
            fonctionExecutiveSearchRepository.search(queryStringQuery(query))
        );
    }
}
