...
public class DistinctionHonorifiqueResource {
    ...
    private final Logger log = LoggerFactory.getLogger(
        DistinctionHonorifiqueResource.class
    );
     ...
     */
    @RequestMapping(
        value = "/distinctionHonorifiques",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public ResponseEntity<DistinctionHonorifique> createDistinctionHonorifique(
        @RequestBody DistinctionHonorifique distinctionHonorifique
    ) throws URISyntaxException {
        log.debug(
            "REST request to save DistinctionHonorifique : {}",
        ...
        if (distinctionHonorifique.getId() != null) {
            return ResponseEntity
                ...
                .header(
                    "Failure",
                    "A new distinctionHonorifique cannot already have an ID"
                )
                .body(null);
        }
        DistinctionHonorifique result = distinctionHonorifiqueRepository.save(
            distinctionHonorifique
        );
        distinctionHonorifiqueSearchRepository.save(result);
        auditTrailService.logCreation(result, result.getId());
        return ResponseEntity
            .created(new URI("/api/distinctionHonorifiques/" + result.getId()))
            .headers(
    ...
    @Timed
    public ResponseEntity<DistinctionHonorifique> updateDistinctionHonorifique(
        @RequestBody DistinctionHonorifique distinctionHonorifique
    ) throws URISyntaxException {
        log.debug(
            "REST request to update DistinctionHonorifique : {}",
            distinctionHonorifique
        );
        if (distinctionHonorifique.getId() == null) {
        ...
        return ResponseEntity
            ...
            .headers(
                HeaderUtil.createEntityUpdateAlert(
                    "distinctionHonorifique",
                    distinctionHonorifique.getId().toString()
                )
            )
            .body(result);
    }
     ...
     */
    @RequestMapping(
        value = "/distinctionHonorifiques/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public ResponseEntity<DistinctionHonorifique> getDistinctionHonorifique(
        @PathVariable Long id
    ) {
        log.debug("REST request to get DistinctionHonorifique : {}", id);
        return Optional
            .ofNullable(distinctionHonorifiqueRepository.findOne(id))
            .map(distinctionHonorifique ->
                new ResponseEntity<>(distinctionHonorifique, HttpStatus.OK)
            )
    ...
    @Timed
    public ResponseEntity<Void> deleteDistinctionHonorifique(
        @PathVariable Long id
    ) {
     ...
     */
    @RequestMapping(
        value = "/_search/distinctionHonorifiques/{query}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public List<DistinctionHonorifique> searchDistinctionHonorifiques(
        @PathVariable String query
    ) {
        return StreamSupport
            .stream(
                distinctionHonorifiqueSearchRepository
