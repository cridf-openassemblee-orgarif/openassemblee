...
public class FonctionGroupePolitiqueResource {
    ...
    private final Logger log = LoggerFactory.getLogger(
        FonctionGroupePolitiqueResource.class
    );
     ...
     */
    @RequestMapping(
        value = "/fonctionGroupePolitiques",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public ResponseEntity<FonctionGroupePolitique> createFonctionGroupePolitique(
        @RequestBody FonctionGroupePolitique fonctionGroupePolitique
    ) throws URISyntaxException {
        log.debug(
            "REST request to save FonctionGroupePolitique : {}",
        ...
        if (fonctionGroupePolitique.getId() != null) {
            return ResponseEntity
                ...
                .header(
                    "Failure",
                    "A new fonctionGroupePolitique cannot already have an ID"
                )
                .body(null);
        }
        FonctionGroupePolitique result = fonctionGroupePolitiqueRepository.save(
            fonctionGroupePolitique
        );
        fonctionGroupePolitiqueSearchRepository.save(result);
        auditTrailService.logCreation(result, result.getId());
        return ResponseEntity
            .created(new URI("/api/fonctionGroupePolitiques/" + result.getId()))
            .headers(
    ...
    @Timed
    public ResponseEntity<FonctionGroupePolitique> updateFonctionGroupePolitique(
        @RequestBody FonctionGroupePolitique fonctionGroupePolitique
    ) throws URISyntaxException {
        log.debug(
            "REST request to update FonctionGroupePolitique : {}",
            fonctionGroupePolitique
        );
        if (fonctionGroupePolitique.getId() == null) {
        ...
        return ResponseEntity
            ...
            .headers(
                HeaderUtil.createEntityUpdateAlert(
                    "fonctionGroupePolitique",
                    fonctionGroupePolitique.getId().toString()
                )
            )
            .body(result);
    }
     ...
     */
    @RequestMapping(
        value = "/fonctionGroupePolitiques/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public ResponseEntity<FonctionGroupePolitique> getFonctionGroupePolitique(
        @PathVariable Long id
    ) {
        log.debug("REST request to get FonctionGroupePolitique : {}", id);
        return Optional
            .ofNullable(fonctionGroupePolitiqueRepository.findOne(id))
            .map(fonctionGroupePolitique ->
                new ResponseEntity<>(fonctionGroupePolitique, HttpStatus.OK)
            )
    ...
    @Timed
    public ResponseEntity<Void> deleteFonctionGroupePolitique(
        @PathVariable Long id
    ) {
     ...
     */
    @RequestMapping(
        value = "/_search/fonctionGroupePolitiques/{query}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public List<FonctionGroupePolitique> searchFonctionGroupePolitiques(
        @PathVariable String query
    ) {
        return StreamSupport
            .stream(
                fonctionGroupePolitiqueSearchRepository
