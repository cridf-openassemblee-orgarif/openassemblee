...
public class MandatureResource {
    ...
    @Timed
    @Secured("ROLE_ADMIN")
    public ResponseEntity<Mandature> createMandature(
        @RequestBody Mandature mandature
    ) throws URISyntaxException {
        log.debug("REST request to save Mandature : {}", mandature);
        if (mandature.getId() != null) {
            return ResponseEntity
                .badRequest()
                .header("Failure", "A new mandature cannot already have an ID")
                .body(null);
        }
        ...
        return ResponseEntity
            .created(new URI("/api/mandatures/" + result.getId()))
            .headers(
    ...
    @Timed
    @Secured("ROLE_ADMIN")
    public ResponseEntity<Mandature> updateMandature(
        @RequestBody Mandature mandature
    ) throws URISyntaxException {
    ...
    public ResponseEntity<Mandature> getMandature(@PathVariable Long id) {
        ...
        return Optional
            .ofNullable(mandatureRepository.findOne(id))
            .map(mandature -> new ResponseEntity<>(mandature, HttpStatus.OK))
    ...
    public ResponseEntity<Void> deleteMandature(@PathVariable Long id) {
        ...
        return ResponseEntity
            .ok()
            .headers(
                HeaderUtil.createEntityDeletionAlert("mandature", id.toString())
            )
            .build();
    }
     ...
     */
    @RequestMapping(
        value = "/_search/mandatures/{query}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    ...
    @RequestMapping(
        value = "/mandatures/set-current/{id}",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    ...
    @RequestMapping(
        value = "/mandatures/set-current-for-session/{id}",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
