...
public class AjouterElusMandatureResource {
    ...
    private final Logger log = LoggerFactory.getLogger(AjouterElusMandatureResource.class);
    ...
    @Timed
    public ResponseEntity<List<CandidatCorrespondanceDTO>> prepareImport(@Valid @RequestBody String csv)
        throws URISyntaxException {
        return ResponseEntity.ok(ajouterElusMandatureService.prepareImport(csv));
    }

    @RequestMapping(value = "/ajouter-elus-mandature/ajouter/{id}",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    @Secured("ROLE_ADMIN")
