...
public class CommissionPermanenteResource {
    ...
    public ResponseEntity<ExecutifDTO> getExecutif() {
        ...
        return new ResponseEntity<>(
            commissionPermanenteService.getExecutif(false),
            HttpStatus.OK
        );
    }
    ...
    public void exportExecutif(HttpServletResponse response) {
        Entry[] entries =
            commissionPermanenteService.getExecutifExportEntries();
        ...
        List<EluEnFonctionDTO> executif = dto
            ...
            .map(fe -> {
                ...
                return new EluEnFonctionDTO(
                    fe.getElu(),
                    listDto.getGroupePolitique(),
                    fe.getFonction()
                );
            })
            .collect(Collectors.toList());
        List<EluEnFonctionDTO> fonctions = dto
            .getFonctions()
            .stream()
            .map(fe -> {
                EluListDTO listDto = eluService.eluToEluListDTO(
    ...
    public ResponseEntity<CommissionPermanenteDTO> getCommissionPermanente() {
        return new ResponseEntity<>(
            commissionPermanenteService.getCommissionPermanente(),
            HttpStatus.OK
        );
    }

    @RequestMapping(
        value = "/commission-permanente/export",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    ...
    public void export(HttpServletResponse response) {
        Entry[] entries =
            commissionPermanenteService.getCommissionPermanenteExportEntries();
        byte[] export = excelExportService.exportToExcel(entries);
    ...
    @Transactional(readOnly = true)
    @RequestMapping(
        value = "/commission-permanente/export-pdf",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    ...
    public void exportPdf(HttpServletResponse response, Authentication auth)
        throws DocumentException {
        List<EluEnFonctionDTO> as =
            commissionPermanenteService.getAppartenancesCommissionPermanenteDtos(
