...
public class MandatResource {
    ...
    @Timed
    @Secured("ROLE_USER")
    public ResponseEntity<Mandat> createMandat(
        @RequestBody MandatEditionDTO dto
    ) throws URISyntaxException {
        log.debug("REST request to save Mandat : {}", dto);
        if (dto.getMandat().getId() != null) {
            return ResponseEntity
                .badRequest()
                .header("Failure", "A new mandat cannot already have an ID")
                .body(null);
        }
    ...
    @Timed
    @Secured("ROLE_USER")
    public ResponseEntity<Mandat> updateMandat(
        @RequestBody MandatEditionDTO dto
    ) throws URISyntaxException {
        log.debug("REST request to update Mandat : {}", dto);
        if (dto.getMandat().getId() == null) {
            throw new IllegalArgumentException(
                dto.getMandat().getId().toString()
            );
        }
        Mandat result = mandatService.save(dto);
    ...
    public ResponseEntity<Mandat> getMandat(@PathVariable Long id) {
        ...
        return Optional
            .ofNullable(mandatRepository.findOne(id))
            .map(mandat -> new ResponseEntity<>(mandat, HttpStatus.OK))
    ...
    public ResponseEntity<Void> deleteMandat(@PathVariable Long id) {
        ...
        return ResponseEntity
            .ok()
            .headers(
                HeaderUtil.createEntityDeletionAlert("mandat", id.toString())
            )
            .build();
    }
