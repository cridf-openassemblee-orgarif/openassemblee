...
public class PouvoirResource {
    ...
    @Timed
    public ResponseEntity<Pouvoir> updatePouvoir(@RequestBody Pouvoir pouvoir)
        throws URISyntaxException {
     ...
     */
    @RequestMapping(
        value = "/pouvoirs/open/{seanceId}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public List<Pouvoir> getAllOpenPouvoirs(@PathVariable Long seanceId)
        throws URISyntaxException {
        Seance s = new Seance();
        s.setId(seanceId);
        return pouvoirRepository.findAllBySeanceAndDateFinAndHeureFin(
            s,
            null,
            null
        );
    ...
    public ResponseEntity<Pouvoir> getPouvoir(@PathVariable Long id) {
        ...
        return Optional
            .ofNullable(pouvoirRepository.findOne(id))
            .map(pouvoir -> new ResponseEntity<>(pouvoir, HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

    @RequestMapping(
        value = "/pouvoirs/close-all",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public void closeAll(@RequestBody PouvoirCloseOrderDTO dto) {
        ...
        pouvoirRepository
            .findAllBySeanceAndDateFinAndHeureFin(s, null, null)
            .forEach(p -> {
    ...
    public ResponseEntity<Void> deletePouvoir(@PathVariable Long id) {
        ...
        return ResponseEntity
            .ok()
            .headers(
                HeaderUtil.createEntityDeletionAlert("pouvoir", id.toString())
            )
            .build();
    }
}
