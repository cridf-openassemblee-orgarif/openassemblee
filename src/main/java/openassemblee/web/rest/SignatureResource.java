...
public class SignatureResource {
    ...
    public ResponseEntity<Signature> createSignature(
        ...
        if (signature.getId() != null) {
            return ResponseEntity
                .badRequest()
                .header("Failure", "A new signature cannot already have an ID")
                .body(null);
        }
    ...
    public ResponseEntity<Signature> getSignature(@PathVariable Long id) {
        ...
        return Optional
            .ofNullable(signatureRepository.findOne(id))
            .map(signature -> new ResponseEntity<>(signature, HttpStatus.OK))
    ...
    public ResponseEntity<Void> deleteSignature(@PathVariable Long id) {
        ...
        return ResponseEntity
            .ok()
            .headers(
                HeaderUtil.createEntityDeletionAlert("signature", id.toString())
            )
            .build();
    }
     ...
     */
    @RequestMapping(
        value = "/_search/signatures/{query}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
