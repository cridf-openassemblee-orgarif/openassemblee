...
public class SeanceResource {
    ...
    public ResponseEntity<Seance> createSeance(
        @RequestBody SeanceCreationDTO seance
    ) throws URISyntaxException {
        log.debug("REST request to save Seance : {}", seance);
        if (seance.getSeance().getId() != null) {
            return ResponseEntity
                .badRequest()
                .header("Failure", "A new seance cannot already have an ID")
                .body(null);
        }
    ...
    public ResponseEntity<List<Seance>> getAllSeances(Pageable pageable)
        throws URISyntaxException {
        Page<Seance> page = seanceRepository.findByMandature(
    ...
    public ResponseEntity<Seance> getSeance(@PathVariable Long id) {
        ...
        return Optional
            .ofNullable(seanceRepository.findOne(id))
            .map(seance -> new ResponseEntity<>(seance, HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

    @RequestMapping(
        value = "/seances/{id}/export-pouvoirs",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public void getPouvoirsExport(
        ...
        for (Pouvoir pv : pouvoirs) {
            Optional<GroupePolitique> groupePolitiqueCedeur = Optional
                .ofNullable(pv.getEluCedeur())
                .flatMap(elu ->
                    elu
            ...
            Optional<GroupePolitique> groupePolitiqueBeneficiaire = Optional
                .ofNullable(pv.getEluBeneficiaire())
                .flatMap(elu ->
                    elu
                        .getAppartenancesGroupePolitique()
                        .stream()
        ...
        ExcelExportService.Entry entry = new ExcelExportService.Entry(
            "Pouvoirs",
            result
        );
    ...
    @RequestMapping(
        value = "/seances/{id}/export-signatures",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public void getSignaturesExport(
        @PathVariable Long id,
        HttpServletResponse response
    ) {
        log.debug("REST request to get Elu : {}", id);
        ...
        for (
            int i = 1;
            i <= (seance != null ? seance.getNombreSignatures() : 0);
            i++
        ) {
        ...
        for (PresenceElu pe : presenceElus) {
            ...
            Optional<GroupePolitique> groupePolitique = pe
                .getElu()
                .getAppartenancesGroupePolitique()
                .stream()
                .filter(GroupePolitiqueService::isAppartenanceCourante)
    ...
    @RequestMapping(
        value = "/seances/{id}/feuille-emargement/{nombreSignatures}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public void getFeuilleEmargement(
        @PathVariable Long id,
        ...
        byte[] export = pdfExportService.feuilleEmargement(
            seance.getDate(),
            elus,
            nombreSignatures
        );
    ...
    public ResponseEntity<SeanceDTO> getSeanceDTO(@PathVariable Long id) {
        log.debug("REST request to get Seance : {}", id);
        return Optional
            .ofNullable(seanceService.getDto(id))
            .map(seance -> new ResponseEntity<>(seance, HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }
    ...
    public ResponseEntity<Void> deleteSeance(@PathVariable Long id) {
        ...
        return ResponseEntity
            .ok()
            .headers(
                HeaderUtil.createEntityDeletionAlert("seance", id.toString())
            )
            .build();
    }
