...
public class ListeElectoraleResource {
     ...
     */
    @RequestMapping(
        value = "/listeElectorales",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    @Secured("ROLE_USER")
    public ResponseEntity<ListeElectorale> createListeElectorale(
        @RequestBody ListeElectorale listeElectorale
        ...
        if (listeElectorale.getId() != null) {
            return ResponseEntity
                ...
                .header(
                    "Failure",
                    "A new listeElectorale cannot already have an ID"
                )
                .body(null);
        }
        ListeElectorale result = listeElectoraleRepository.save(
            listeElectorale
        );
        listeElectoraleSearchRepository.save(result);
        return ResponseEntity
            .created(new URI("/api/listeElectorales/" + result.getId()))
            .headers(
    ...
    @Timed
    @Secured("ROLE_USER")
    public ResponseEntity<ListeElectorale> updateListeElectorale(
        @RequestBody ListeElectorale listeElectorale
    ) throws URISyntaxException {
        log.debug(
            "REST request to update ListeElectorale : {}",
            listeElectorale
        );
        if (listeElectorale.getId() == null) {
        ...
        return ResponseEntity
            ...
            .headers(
                HeaderUtil.createEntityUpdateAlert(
                    "listeElectorale",
                    listeElectorale.getId().toString()
                )
            )
            .body(result);
    }
    ...
    public List<ListeElectorale> getAllListeElectorales() {
        ...
        return listeElectoraleRepository.findByMandature(
     ...
     */
    @RequestMapping(
        value = "/listeElectorales/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public ResponseEntity<ListeElectorale> getListeElectorale(
        @PathVariable Long id
    ) {
        log.debug("REST request to get ListeElectorale : {}", id);
        return Optional
            .ofNullable(listeElectoraleRepository.findOne(id))
            .map(listeElectorale ->
                new ResponseEntity<>(listeElectorale, HttpStatus.OK)
            )
     ...
     */
    @RequestMapping(
        value = "/_search/listeElectorales/{query}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    @Timed
    public List<ListeElectorale> searchListeElectorales(
        @PathVariable String query
    ) {
        return StreamSupport
            .stream(
                listeElectoraleSearchRepository
