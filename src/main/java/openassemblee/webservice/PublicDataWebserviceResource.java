...
public class PublicDataWebserviceResource {
    ...
    public Map<String, Object> websitedata() {
        ...
        List<Elu> elus = eluRepository
            .findAll()
            .stream()
            .filter(e -> isCurrentMandat(e.getMandats(), mandature))
            .collect(Collectors.toList());
        List<GroupePolitique> groupesPolitiques =
            groupePolitiqueRepository.findByMandature(mandature);
        List<CommissionThematique> commissionsThematiques =
            commissionThematiqueRepository.findByMandature(mandature);
        ...
        ensembles.addAll(
            getEnsemblesCommissionsThematiques(commissionsThematiques)
        );
    ...
    private List<ConseillerDto> getConseillers(List<Elu> elus) {
        return elus
            ...
            .map(e -> {
                ...
                getPublishable(e.getAdressesPostales())
                    .ifPresent(ap -> {
                ...
                Optional<AppartenanceGroupePolitique> agp = e
                    ...
                    .findFirst();
                Optional<FonctionGroupePolitique> fgp = e
                ...
                d.setTelephone(
                    getPublishable(e.getNumerosTelephones())
                        .map(NumeroTelephone::getNumero)
                        .orElse(SPACE)
                );
                d.setFax(
                    getPublishable(e.getNumerosFax())
                        .map(NumeroFax::getNumero)
                        .orElse(SPACE)
                );
                d.setMail(
                    getPublishable(e.getAdressesMail())
                ...
                if (e.getCivilite() != null) {
                    d.setCivilite(
                        e.getCivilite() == Civilite.MONSIEUR ? "M." : "Mme"
                    );
                }
                ...
                e
                    ...
                    .forEach(a ->
                        commissionsStringBuilder
                            .append("|")
                            .append(COMMISSION_PERMANENTE)
                    );
                ...
                e
                    ...
                    .forEach(a ->
                        commissionsStringBuilder
                            .append("|")
                            .append(a.getCommissionThematique().exportUid())
                    );
                d.setCommissions(
                    IS_TEST_IMPORT
                        ? EMPTY_STRING
                        : commissionsStringBuilder.toString()
                );
                d.setDesignations(EMPTY_STRING);

                d.setDistinctions(SPACE);
                if (
                    e.getDistinctionHonorifiques().size() > 0 && !IS_TEST_IMPORT
                ) {
                    d.setDistinctions(
                        "|" +
                        String.join(
                            ...
                            e
                                ...
                                .stream()
                                .map(dh ->
                                    stringOrEmpty(dh.getTitre()) +
                                    "$" +
                                    stringOrEmpty(dh.getDate())
                                )
                ...
                Optional<FonctionExecutive> fe = e
                    ...
                    .findFirst();
                d.setFonctionExecutif(
                    stringOrSpace(
                        fe
                            .map(fonctionExecutive ->
                                fonctionExecutive.getFonction()
                            )
                            .orElse(null)
                    )
                );
    ...
    private List<EnsembleDto> getCommissionPermanente() {
        ...
        cp.setLibCourt("CP");
        cp.setDateCreation(
            formatDate(sessionMandatureService.getMandature().getDateDebut())
        );
    ...
    private List<EnsembleDto> getEnsemblesGroupesPolitiques(
        ...
        return groupePolitiques
            ...
            .map(gp -> {
                ...
                if (gp.getAdressePostale() != null) {
                    e.setAdresse(
                        stringOrSpace(gp.getAdressePostale().getVoie())
                    );
                    e.setCodePostal(
                        stringOrSpace(gp.getAdressePostale().getCodePostal())
                    );
                    e.setVille(
                        stringOrSpace(gp.getAdressePostale().getVille())
                    );
                }
    ...
    private List<EnsembleDto> getEnsemblesCommissionsThematiques(
                ...
                e
                    ...
                    .map(fcp -> {
                        ...
                        if (fcp.getFonction().contains("résident")) {
                            ...
                            m.setBureau('0');
                            if (
                                fcp.getFonction().equals("Président") ||
                                fcp.getFonction().equals("Présidente")
                            ) m.setBureau('1');
                        } else {
            ...
            .flatMap(e ->
                e
                    ...
                    .map(fct -> {
                        ...
                        m.setUidEnsemble(
                            fct.getCommissionThematique().exportUid()
                        );
                        m.setUidConseiller(e.exportUid());
        ...
        List<MembreDto> m4 = elus
            ...
            .flatMap(e ->
                e
                    ...
                    .filter(a -> a.getDateFin() == null)
                    .filter(act ->
                        !fonctionsCT.contains(act.exportUid()) &&
                        act.getCommissionThematique() != null
                    )
                    .map(act -> {
                        MembreDto m = new MembreDto();
                        m.setUidMembre(act.exportUid());
                        m.setUidEnsemble(
                            act.getCommissionThematique().exportUid()
                        );
                        m.setUidConseiller(e.exportUid());
        ...
        List<MembreDto> m6 = elus
            ...
            .flatMap(e ->
                e
                    ...
                    .filter(a -> a.getDateFin() == null)
                    .filter(agp ->
                        !fonctionsGP.contains(agp.exportUid()) &&
                        agp.getGroupePolitique() != null
                    )
                    .map(agp -> {
                        MembreDto m = new MembreDto();
    ...
    private <T extends Publishable> Optional<T> getPublishable(
        List<T> publishables
    ) {
        return publishables
            .stream()
            .filter(n ->
                n.getPublicationAnnuaire() != null && n.getPublicationAnnuaire()
            )
    ...
    private String getUrl(Elu e, TypeIdentiteInternet typeIdentiteInternet) {
        Optional<IdentiteInternet> ii = e
    ...
    private char bureau(String fonction) {
        if (fonction == null) return '0';
        // TODO une exception ?
        if (fonction.contains("Présidente du Conseil Régional")) return '0';
        // TODO une exception ?
        if (
            fonction.contains(
                "13ème Vice-Président chargé du logement et de la politique de la ville"
            )
        ) return '0';
        // TODO une exception ?
        if (fonction.contains("Présidente déléguée")) return '0';
        // TODO une exception ?
        if (
            fonction.contains(
                "9ème Vice-Président chargé de l'action internationale et du tourisme"
            )
        ) return '0';
        // .contains("résident") pour président, vice-président, présidente, avec ou sans maj...
        if (fonction.contains("résident")) return '1';
        if (fonction.equals("Secrétaire")) return '1';
        return '0';
    }
}
