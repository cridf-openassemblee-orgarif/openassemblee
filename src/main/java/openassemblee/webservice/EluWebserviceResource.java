...
public class EluWebserviceResource {
    ...
    class PublicElu {
        ...
        public PublicElu(
            String id,
            String uid,
            PublicCivilite civilite,
            String nom,
            String prenom,
            String groupePolitique,
            String groupePolitiqueShort,
            String image,
            Boolean actif
        ) {
            this.id = id;
    ...
    public Data elus() {
        List<PublicElu> elus = eluService
            .getAll(false, false, false)
            .stream()
            .map(it -> {
                String groupePolitique = it.getGroupePolitique() != null
                    ? it.getGroupePolitique().getNom()
                    : "sans groupe";
                String groupePolitiqueCourt = it.getGroupePolitique() != null
                    ? it.getGroupePolitique().getNomCourt()
                    : "-";
                return new PublicElu(
                    it.getElu().getShortUid().toString(),
                    it.getElu().getUid().replace("-", ""),
                    it.getElu().getCivilite() == Civilite.MONSIEUR
                        ? PublicCivilite.M
                        : PublicCivilite.MME,
                    it.getElu().getNom(),
                    it.getElu().getPrenom(),
                    ...
                    it.getElu().getImage() != null
                        ? "/images/" + it.getElu().getImage()
                        : null,
                    isCurrentMandat(
                        it.getElu().getMandats(),
